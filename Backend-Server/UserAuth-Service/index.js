...
app.post("/auth/register", async (req, res) => {
  const { name, email, phone, password, repassword } = req.body; //we will get email, pass, and name from the client

  const userExists = await User.findOne({ email }); //Authenticate if the user exists using email
  if (userExists) {
...
app.get("/auth/users", async (req, res) => {
  try {
    const cachedUsers = await getOrSetCache("users", async () => {
